...
$(function () {
    ...
    $('.audit-filter').on('click', function (e) {
        debugger
        var lengthOfCheckBox = $("input.audit-filter:checkbox:not(:checked)").length;
        var possitionAudit = $("#auditCollapseOne").data("audit");
        if (lengthOfCheckBox === 2 && possitionAudit !== "MdClient")
            table_.search("").draw();
        else if (lengthOfCheckBox === 5 && possitionAudit === "MdClient")
            table_.search("").draw();
        else if (lengthOfCheckBox === 6 && possitionAudit === "MdPayorConfig")
            table_.search("").draw();
        else {
            ...
            else if (possitionAudit === "MdClient") {
                add = "AddClient".toLowerCase();
                edit = "EditClient".toLowerCase();
                addA = "AddAttachment".toLowerCase();
                deleteA = "DeleteAttachment".toLowerCase();
                addN = "AddNote".toLowerCase();
                
            }
            else if (possitionAudit === "MdPayorConfig") {
                add = "AddPayerConfig".toLowerCase();
                edit = "EditPayerConfig".toLowerCase();
                del = "DeletePayerConfig".toLowerCase();
                addA = "AddAttachment".toLowerCase();
                deleteA = "DeleteAttachment".toLowerCase();
                addN = "AddNote".toLowerCase();
            ...
            $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
                ...
                if ($("#auditCollapseOne #auditFilterXoptC").is(":checked")) {
                    if (~data[2].toLowerCase().indexOf(addA)) return true;
                }
                if ($("#auditCollapseOne #auditFilterXoptD").is(":checked")) {
                    if (~data[2].toLowerCase().indexOf(deleteA)) return true;
                }
                if ($("#auditCollapseOne #auditFilterXoptE").is(":checked")) {
                    if (~data[2].toLowerCase().indexOf(addN)) return true;
                }
                if ($("#auditCollapseOne #auditFilterXoptF").is(":checked")) {
                    if (~data[2].toLowerCase().indexOf(del)) return true;
                }
